 ...
 */
public class ID3v2FrameFake implements MP3Frame {
    ...
    public static MP3Frame createFrameIfPresent(InputStream inp)
        ...
        if (h1 == (int)'I' && h2 == (int)'D' && h3 == (int)'3') {
            ...
            return new ID3v2FrameFake(majorVersion, minorVersion, inp);
        }
    ...
    private ID3v2FrameFake(int majorVersion, int minorVersion, InputStream inp)
            throws IOException {
    ...
    protected static String getTagString(byte[] data, int offset, int length) {
        return getTagString(data, offset, length, "windows-1251");
    }
    protected static String getTagString(byte[] data, int offset, int length, String defEnc) {
        ...
        try {
            ...
            return new String(data, offset, actualLength, enc);
    ...
    protected static ID3Comment getComment(byte[] data, int offset, int length) {
        return getComment(data, offset, length, "windows-1251");
    }
    protected static ID3Comment getComment(byte[] data, int offset, int length, String defenc) {
       ...
       try {
          for (int i=descStart; i<offset+length; i++) {
             if (encoding.doubleByte && data[i]==0 && data[i+1] == 0) {
                ...
                textStart = i+2;
                description = new String(data, descStart, i-descStart, enc);
                break;
             }
          ...
          // Did we find the end?
          if (textStart > -1) {
             text = new String(data, textStart, offset+length-textStart, enc);
          } else {
             // Assume everything is the text
             text = new String(data, descStart, offset+length-descStart, enc);
          }
    ...
    protected static String getString(byte[] data, int offset, int length) {
        try {
            return new String(data, offset, length, "windows-1251");
        } catch (UnsupportedEncodingException e) {
